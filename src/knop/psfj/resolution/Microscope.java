...
public class Microscope {
	...
	public static String BEAD_SIZE_KEY = "beadsize";
	...
	public Microscope() {
		...
		;
		beadSize = prefs.getDouble(BEAD_SIZE_KEY, 0.0);
	...
	public Microscope(IniFile ini) {
		...
		beadSize = config.getDoubleValue(MICROSCOPE_SECTION, BEAD_SIZE_KEY,
				prefs.getDouble(BEAD_SIZE_KEY, 0.0));
	...
	*/
		if (config != null) {
			...
			config.addStringValue(MICROSCOPE_SECTION, BEAD_SIZE_KEY,
					MathUtils.roundToString(beadSize, 4));
	...
	public String getMicroscopeHeader() {
		...
		reportHeader += "Emission wavelength: " + MathUtils.formatDouble(wavelength,getUnit()) + " "
				...
				+ "\nBead diameter : " + formatDouble(beadSize) + " "
				+ "\n\n";
		return reportHeader;
	}
	...
	public double getXYTheoreticalResolutionWithBeadSize() {
		calculateTheoreticalResolutions();
		return Math.sqrt(Math.pow(beadSize, 2.5) + Math.pow(resolution[0], 2));
	}

	public double getZTheoreticalResolutionWithBeadSize() {
		calculateTheoreticalResolutions();
		return Math.sqrt(Math.pow(beadSize, 2.5) + Math.pow(resolution[2], 2));
	}
	...
	public double getCorrectedResolution(double fwhm, int axe) {
		...
		return fwhm * getAlpha(fwhm, expectedFWHM, beadSize, axe);
