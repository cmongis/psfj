...
import Objects3D.Coordinates3D;
import Objects3D.Object3D;
import Objects3D.Voxel;
...
public class Counter3D extends Observable{
   ...
   public Counter3D(ImagePlus img, int thr, int min, int max) {
	   this(img,thr,min,max,true,false);
   }
   ...
   public void setSizeFiltering(boolean b) {
   	sizeFilter = b;
   }
   ...
   private void findObjects() {
       ...
       for (int z=1; z<=nbSlices; z++){
           ...
           //setStatus("Finding structures "+z*100/nbSlices+"%");
           setStatus("Step 1/3: Finding structures");
           setProgress(z, nbSlices);
       }
       setStatus("");
       ...
       for (int z=1; z<=nbSlices; z++){
           ...
           setStatus("Step 2/3: Connecting structures");
           setProgress(z, nbSlices);
       }
       setStatus("");
       ...
       for (int i=1; i<IDcount.length; i++){
           ...
           setStatus("Step 3/3: Renumbering structures");
           setProgress(i, currID);
       }
       setStatus("");
   ...
   public void getObjects(){
       ...
       if (!getObjects){
           ...
           for (int i=0; i<nbObj; i++) obj.add(new Object3D(cal));
           IDcount=null;
    ...
    *
    * @return the coordinates of all found centro�ds as a dual float array ([ID][0:x, 1:y, 2:z]).
    */
   ...
   private void populateSurfPixCoord(){
       ...
       surfCoord=new Voxel[nbSurfPix];
       
       for (int i=0; i<nbObj; i++){
           Object3D currObj=(Object3D) obj.get(i);
           Vector<Coordinates3D> surfVox=currObj.surf_voxelsCoord;
           for (int j=0; j<surfVox.size(); j++){
               surfCoord[index]=new Voxel(surfVox.elementAt(j), i++, true);
               index++;
           }
       }
       calcStats();
   ...
   public void showStatistics(boolean newRT){
       ...
       for (int i=0; i<nbObj; i++){
           ...
           rt.setLabel(WindowManager.getCurrentImage().getTitle()+"_Object_"+(i+1), rt.getCounter()-1);
           
           if (Prefs.get("3D-OC-Options_volume.boolean", true)) rt.setValue("Volume ("+unit+"^3)", rt.getCounter()-1, currObj.obj_voxels.size()*calXYZ);
           if (Prefs.get("3D-OC-Options_surface.boolean", true)) rt.setValue("Surface ("+unit+"^2)", rt.getCounter()-1, currObj.surf_cal);
           if (Prefs.get("3D-OC-Options_objVox.boolean", true)) rt.setValue("Nb of obj. voxels", rt.getCounter()-1, currObj.obj_voxels.size());
           if (Prefs.get("3D-OC-Options_surfVox.boolean", true)) rt.setValue("Nb of surf. voxels", rt.getCounter()-1, currObj.surf_size);
           if (Prefs.get("3D-OC-Options_IntDens.boolean", true)) rt.setValue("IntDen", rt.getCounter()-1, currObj.int_dens);
           if (Prefs.get("3D-OC-Options_mean.boolean", true)) rt.setValue("Mean", rt.getCounter()-1, currObj.mean_gray);
           if (Prefs.get("3D-OC-Options_stdDev.boolean", true)) rt.setValue("StdDev", rt.getCounter()-1, currObj.SD);
           if (Prefs.get("3D-OC-Options_median.boolean", true)) rt.setValue("Median", rt.getCounter()-1, currObj.median);
           if (Prefs.get("3D-OC-Options_min.boolean", true)) rt.setValue("Min", rt.getCounter()-1, currObj.min);
           if (Prefs.get("3D-OC-Options_max.boolean", true)) rt.setValue("Max", rt.getCounter()-1, currObj.max);
           
           
           if (Prefs.get("3D-OC-Options_centroid.boolean", true)){
               float[] tmpArray=currObj.centroid;
               rt.setValue("X", rt.getCounter()-1, tmpArray[0]);
               rt.setValue("Y", rt.getCounter()-1, tmpArray[1]);
               if (nbSlices!=1) rt.setValue("Z", rt.getCounter()-1, tmpArray[2]);
           }
           
           if (Prefs.get("3D-OC-Options_meanDist2Surf.boolean", true)) rt.setValue("Mean dist. to surf. ("+unit+")", rt.getCounter()-1, currObj.mean_dist2surf);
           if (Prefs.get("3D-OC-Options_SDDist2Surf.boolean", true)) rt.setValue("SD dist. to surf. ("+unit+")", rt.getCounter()-1, currObj.SD_dist2surf);
           if (Prefs.get("3D-OC-Options_medDist2Surf.boolean", true)) rt.setValue("Median dist. to surf. ("+unit+")", rt.getCounter()-1, currObj.median_dist2surf);
           
           if (Prefs.get("3D-OC-Options_COM.boolean", true)){
               float[] tmpArray=currObj.c_mass;
               rt.setValue("XM", rt.getCounter()-1, tmpArray[0]);
               rt.setValue("YM", rt.getCounter()-1, tmpArray[1]);
               if (nbSlices!=1) rt.setValue("ZM", rt.getCounter()-1, tmpArray[2]);
           }
           
           if (Prefs.get("3D-OC-Options_BB.boolean", true)){
           int[] tmpArrayInt=currObj.bound_cube_TL;
               rt.setValue("BX", rt.getCounter()-1, tmpArrayInt[0]);
               rt.setValue("BY", rt.getCounter()-1, tmpArrayInt[1]);
               if (nbSlices!=1) rt.setValue("BZ", rt.getCounter()-1, tmpArrayInt[2]);

               rt.setValue("B-width", rt.getCounter()-1, currObj.bound_cube_width);
               rt.setValue("B-height", rt.getCounter()-1, currObj.bound_cube_height);
               if (nbSlices!=1) rt.setValue("B-depth", rt.getCounter()-1, currObj.bound_cube_depth);
           }
   ...
   public void showSurfPix(){
       ...
       for (int i=0; i<surfCoord.length; i++){
           rt.incrementCounter();
           rt.setValue(0, i, surfCoord[i].val);
           rt.setValue(1, i, surfCoord[i].x);
           rt.setValue(2, i, surfCoord[i].y);
           rt.setValue(3, i, surfCoord[i].z);
       }
   ...
   private void calcStats() {
       for (int i=0; i<obj.size(); i++) obj.elementAt(i).calcStats();
   }
	...
	public void setProgress(int progress,int total) {
		setChanged();
		//notifyObservers(stack.getProcessor(currentZ));
