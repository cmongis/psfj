...
public class BeadImage extends Observable {
	...
	/**  When auto calculating the automatic frame size will use beadEnlargementFactor * sigma in pixel. */
	...
	/**
	 * Gets the raw focus point, without launching the auto calculation algorithm
	 * if the focus point is not set.
	 * 
		...
		if (beadLocation == null) {
			...
			for (int i = 4; i <= 30 && blobs == null; i += 3) {
				try {
					...
					System.out.println(getMiddleImage().getMax());

					System.err.println("Detection error.\nThreshold :"
							+ getThresholdValue());
					setProgress(0, "Detection error.... raising threshold...");
					setThresholdValue(getThresholdValue() + i);
	 ...
	 * 
	 * @param wavelength
	 *           the new wave length in nano meters
	 */
	 ...
	 * function uses the result of the getBeadLocation, and create a frame around
	 * each object. When two frames are overlapping too much, only the frame
	 * containing the brightest bead is kept.
	 * 
	...
	public synchronized BeadFrameList getBeadFrameList() {
		if (beadFrames == null) {
				...
				for (int j = 0; j != total; j++) {
					...
					if (r != r2) {
						...
						if (r2.contains(r.getCenterX(), r.getCenterY())
							...
							double maxR1 = getBeadMaxIntensity(getEnlargedFrame(
									getBeadLocation().get(i), 1));
							double maxR2 = getBeadMaxIntensity(getEnlargedFrame(
			...
			ignoredFrameNumber = toRemove.size();

			setFrameNumber(beadFrames.size());
	 ...
	 *           the source image
	 * @param r
	 *           the rectangle represanting the ROI
		...
		if (preview == null) {
			try {
				...
				for (BeadFrame frame : getBeadFrameList()) {
					...
					if(frame.isValid() == false) r = getEnlargedFrame((Rectangle)r, MathUtils.round(r.getWidth()/2));
	 ...
	 * 
	 * @return nothing : they can be accessed via the attributes minThreshold and
	 *         maxThreshold.
	 */
	 ...
	 * 
	 * @param beadEnlargement
	 *           the new bid enlargement
	 */
	 ...
	 * 
	 * @param ip
	 *           the image processing that the view is suggested to display
	 * @param name
	 *           the name of the message sent to the observers
	 */
	 ...
	 * @param strData
	 *           the string data inside the message (e.g : "Doing something...")
	 * @param intData
	 *           the int data inside the message (e.g : 10 %)
	 * @param data
	 *           the data
	 */
...
System.out.println(String.format(
	...
	public String getInvalidyReason() {
		if(this.invalidityReason != null) return this.invalidityReason;
		return null;
	}
	...
	public void setInvalidityReason(String reason) {
		if(invalidityReason == null) invalidityReason = reason;
	}
