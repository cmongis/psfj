...
public class GaussianUtils {
 ...
 /**
   ...
   public static double[] ellipseParmConversion(double a, double b, double c) {
     ...
     // theta = 0;
      newA = (sqr(costheta)/2/sqr(sx)) + (sqr(sintheta)/2/sqr(sy));
      newB = (Math.sin(2*theta)/4/sqr(sx)) + (Math.sin(2*theta)/4/sqr(sy));
      newC = (sqr(sintheta)/2/sqr(sx)) + (sqr(costheta)/2/sqr(sy));
     */
     ...
     // theta = Math.PI/2 - theta;
      ...
      double t = Math.sqrt(sqr(b)+sqr(a-c));
      double sx = Math.sqrt(1/(a+t+c));
      double sy = Math.sqrt(1/(a-t+c));
      double theta = Math.sqrt(1-((a-c)/t))/Math.sqrt(2);
      theta = Math.acos(theta);
      
      */
      	
      //result[0] = Math.signum(b)*(Math.PI/2) - theta;
      result[0] = theta;
      result[1] = sx;
      result[2] = sy;
      	
     // System.out.println(String.format(" a/a = %.3f\n b/b = %.3f\n c/c = %.3f",a/newA,b/newB,c/newC));
